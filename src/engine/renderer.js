...
export class Renderer {
  constructor(canvas, quality = 'med') {
    ...
    this.bloom = new UnrealBloomPass(new THREE.Vector2(1, 1), quality === 'low' ? 0.22 : 0.3, 0.45, 0.95);
    this.composer.addPass(this.bloom);
    this.grade = new ShaderPass(GradeShader);
    this.composer.addPass(this.grade);
    this.composer.addPass(new OutputPass());
    if (quality !== 'low') { this.smaa = new SMAAPass(); this.composer.addPass(this.smaa); }
    this.resize();
  ...
  // adaptive resolution: drop render scale if frame time is bad
  adapt(dt, now) {
    this.fpsHist.push(dt); if (this.fpsHist.length > 60) this.fpsHist.shift();
    if (now - this.lastAdapt < 2500 || this.fpsHist.length < 50) return;
    const avg = this.fpsHist.reduce((a, b) => a + b, 0) / this.fpsHist.length;
    let s = this.dynamicScale;
    if (avg > 1 / 40 && s > 0.6) s -= 0.1; else if (avg < 1 / 75 && s < 1) s += 0.1;
